...
namespace Pseudo.Mechanics.Internal
{
	...
	public class ComboSystemEditor : PEditor
	{
		...
		void ShowComboEnums()
		{
			...
			Popup(comboManagerProperty.FindPropertyRelative("inputEnumName"), ComboEnumTypeDict.Keys.ToArray(), "Input Enum".ToGUIContent());
		...
		void ShowComboSequence()
		{
			...
			if (currentSequenceProperty.isExpanded)
			{
				...
				UniqueNameField(currentSequence, comboManager.combos);
		...
		{
			if (AddFoldOut(sequenceItemsProperty, "Sequence".ToGUIContent()))
			{
