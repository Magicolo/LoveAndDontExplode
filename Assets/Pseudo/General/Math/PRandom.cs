...
namespace Pseudo
{
	public static class PRandom
	{
		...
		public static double NextInversedGaussian()
		{
			...
			{
				// F^-1(p) = - G^-1(p)
				bob = -RationalApproximation(Math.Sqrt(-2.0 * Math.Log(p)));
			...
			{
				// F^-1(p) = G^-1(1-p)
				bob = RationalApproximation(Math.Sqrt(-2.0 * Math.Log(1.0 - p)));
			}

			return PMath.Clamp(bob, 0d, 1d);
		...
		public static double Range(double min, double max, ProbabilityDistributions distribution)
		{
			...
			switch (distribution)
			{
				...
				case ProbabilityDistributions.Gaussian:
					...
					break;
				case ProbabilityDistributions.InversedGaussian:
					randomValue = NextInversedGaussian();
					break;
			}
